...
#x0 =[0.058955250991610135,0.019552516188244648,0.017681469946090211,0.00000000067]# b1, b2, tow
x0=[0.1,0.1,0.1,1] #b0, b1, b2, t1
...
def woo(list):
    ...
    for i in np.arange(len(nss)):
        dm[i]=np.exp(-nss.iloc[i]*(df['MATURITY'].iloc[i]))
    ...
    for i in np.arange(len(nss)):
        pmt=pd.Series([[]])
        a=pd.Series([[]])
        for j in np.arange(round(df['MATURITY'].iloc[i])/0.5):
            a[j]=np.exp(-(df['MATURITY'].iloc[i]-0.5*j)*nss.iloc[i,0])*(df['CPN'].iloc[i]/2)
        pmt[i] = a.sum() if a.sum() else 0
#        print ("i:",i,"pmt:",pmt[i])
 #       print ("i:",i,"back:",np.exp(-nss.iloc[i,0]*df['MATURITY'].iloc[i])*100)
        estprice[i]=pmt[i]+np.exp(-nss.iloc[i,0]*df['MATURITY'].iloc[i])*100   
    pi=df['DUR_ADJ_MID']*df['PX_LAST']/(1+df['YLD_YTM_MID']/100)
    diff=np.power((df['PX_LAST']-estprice)/pi,2)
    return diff.sum()

a=sp.optimize.minimize(woo,x0,method='Nelder-Mead')
p=a['x']
...
def ram(list):
    ...
    for i in np.arange(len(svs)):
        ...
        pmt[i] = a.sum() if a.sum() else 0
#        print ("i:",i,"pmt:",pmt[i])
#        print ("i:",i,"back:",np.exp(-svs.iloc[i,0]*df['MATURITY'].iloc[i])*100)
...
b=sp.optimize.minimize(ram,x1,method='Nelder-Mead')
q=b['x']
SVS=[]
for i in np.arange(0,len(dff)):
...
b=sp.optimize.minimize(wr,x1,method='Nelder-Mead')
q=b['x']
svs_dff=pd.DataFrame(np.linspace(0,30,101), columns=['MATURITY'])
SVS=[]
for i in np.arange(0,len(dff)):
    x=q[0]+q[1]*(1-np.exp(-svs_dff['MATURITY'][i]/q[4]))*(-q[4]/svs_dff['MATURITY'][i])+q[2]*((1-np.exp(-svs_dff['MATURITY'][i]/q[4]))*(q[4]/svs_dff['MATURITY'][i])-np.exp(-svs_dff['MATURITY'][i]/q[4]))+q[3]*((1-np.exp(-svs_dff['MATURITY'][i]/q[5]))*(q[5]/svs_dff['MATURITY'][i])-np.exp(-svs_dff['MATURITY'][i]/q[5]))
    SVS.append(x)
SVS=pd.DataFrame(SVS)
plt.plot(dff['MATURITY'],NS, label='Nelson-Siegel')
plt.plot(dff['MATURITY'],SVS, label='SVS')
plt.legend(loc='upper left')